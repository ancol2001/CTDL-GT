...
{ // biểu diễn lớp single_linked_list
public:
...
{                             // tạo node rời rạc có giá trị value
    ...
    if (temp == NULL)
    { // nếu không đủ gian nhớ để cấp phát
...
{              // thêm node vào đầu
    ...
    if (start == NULL)
    {                       // nếu danh sách liên kết đơn rỗng
...
{              // thêm node vào cuối danh sách liên kết đơn
    ...
    while (s->next != NULL)
    { // di chuyển s đến node cuối cùng của DSLKĐ
        s = s->next;
    }
...
{ // Them node vao vi tri cho truoc
    ...
    s = start; // s trỏ đến start
    while (s != NULL)
    { // đếm số node của DSLKĐ là counter
        s = s->next;
        counter++;
    }
    if (pos == 1)
    { // nếu pos là node đầu tiên
        if (start == NULL)
        {                       // trường hợp DSLKĐ rỗng
            start = temp;       // start trỏ đến temp
            start->next = NULL; // thiết lập liên kết cho start là null
        }
        else
        {                      // trường hợp DSLKĐ không rỗng
    ...
    {              // trường hợp vị trí pos hợp lệ
        s = start; // s trỏ đến start
        for (i = 1; i < pos; i++)
        { // di chuyển ptr đến node sau vị trí pos
            ptr = s;
            s = s->next;
        }
    ...
    { // trường hợp vị trí pos không hợp lệ
...
{ // chương trình chính
    ...
    while (1)
    {
        ...
        case 0:
            ...
            exit(1);
            break;
            default : cout << "Lựa chọn sai" << endl;
            break;
        }
    }
}
